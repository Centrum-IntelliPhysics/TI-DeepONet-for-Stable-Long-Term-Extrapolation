...
inputs = outputs_u[:, 0, :, :, None]     #(Ns, Nx, Ny, Nv=1)
outputs = outputs_u[:, :tt, :, :, None]    #(Ns, Nt, Nx, Ny, Nv=1)
...
auto_reg_error = []
for t in range(outputs_u.shape[1]):
    err_val = np.linalg.norm(pred_FNO[:,t,...,0] - outputs_u[:,t,...])/np.linalg.norm(outputs_u[:,t,...])
