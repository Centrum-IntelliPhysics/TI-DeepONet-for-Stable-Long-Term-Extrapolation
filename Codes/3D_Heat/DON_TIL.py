...
def dynamic_rk4_step(u_curr, model_fn, params, model_rk_fn, rk_params, trunk_inputs, dt):
    ...
    #Extract the coefficients  - each with shape (batch_size, 1) and a new axes dimension for broadcasting in adaptive RK4 update
...
def update(params, rk_params, branch_inputs, trunk_inputs, gt_outputs, opt_state, opt_state_rk, dt):
    ...
    (loss,_), grads = jax.value_and_grad(loss_fn, argnums = (0,1), has_aux=True)(params, rk_params, 
...
rk_lr_scheduler = optax.schedules.exponential_decay(init_value=5e-3, transition_steps=2000, decay_rate=0.96)
